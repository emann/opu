...
impl TryFrom<PathBuf> for OP1Dirs {
    ...
    fn try_from(parent_dir: PathBuf) -> Result<Self, Self::Error> {
        ...
        album
            .try_exists()
            .map_err(|_| Error::MissingDir("album".to_owned()))?;

        let drum = parent_dir.join("drum");
        drum.try_exists()
            .map_err(|_| Error::MissingDir("drum".to_owned()))?;

        let synth = parent_dir.join("synth");
        synth
            .try_exists()
            .map_err(|_| Error::MissingDir("synth".to_owned()))?;

        let tape = parent_dir.join("tape");
        tape.try_exists()
            .map_err(|_| Error::MissingDir("tape".to_owned()))?;
